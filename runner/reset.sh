...
# Enable gateway
if [ $POWER_EN_GPIO -ne 0 ]
then
    echo "Concentrator enabled through GPIO$POWER_EN_GPIO"
    if [ -d /sys/class/gpio/gpio$POWER_EN_GPIO ]
    then
    ...
    echo 1 > /sys/class/gpio/gpio$POWER_EN_GPIO/value; WAIT_GPIO
    echo $POWER_EN_GPIO > /sys/class/gpio/unexport; WAIT_GPIO
fi

# Reset gateway
echo "Concentrator reset through GPIO$RESET_GPIO"
if [ -d /sys/class/gpio/gpio$RESET_GPIO ]
then
