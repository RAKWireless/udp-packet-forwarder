...
**`HAS_GPS`** | 0 or 1 | Set to 0 to disable onbard GPS | If `GPS_DEV` is defined it will be set to 1 by default
...
**`TTN_REGION`** | `STRING` | **Deprecated, use TTS_REGION instead** |
**`GATEWAY_EUI_NIC`** | `STRING` | **Deprecated, use GATEWAY_EUI_SOURCE instead** |
**`RADIO_DEV`** | `STRING` or `AUTO` | **Deprecated, use DEVICE instead** |

Notes: 

> At least `MODEL` must be defined.

> The list of supported modules is at the top of this page (either RAK Wisgate Developer model numbers or RAK WisLink modules). If your device is not in the list you can manually define `CONCENTRATOR`, `INTERFACE`, `GPS_DEV`, `HAS_LTE` and `RESET_GPIO`.
